---
title: "카카오 로그인 SDK v2 마이그레이션 가이드: Next.js에서 인앱브라우저 이슈 해결하기"
description: "인스타그램 인앱브라우저에서 카카오 로그인이 작동하지 않는 문제를 해결하기 위해 카카오 SDK v1에서 v2로 마이그레이션한 과정을 상세히 설명합니다. Next.js 환경에서 카카오 SDK v2 적용 방법, 로그인 플로우 구현, 쿠키 기반 인증 처리까지 실제 개발 경험을 공유합니다."
pubDate: "12 01 2024"
keywords: "카카오 로그인, 카카오 SDK, 카카오 SDK v2, 인앱브라우저, Next.js 카카오 로그인, 카카오 로그인 구현, 카카오 SDK 마이그레이션, 프론트엔드, 웹 개발"
tags: ["FE", "카카오", "로그인", "Next.js"]
---
이 글은 인앱브라우저 이슈로 인해 카카오 로그인 SDK version1에서 version2로 업그레이드한 내용을 담고 있습니다. **카카오 로그인 SDK v2**를 **Next.js** 환경에서 구현하는 방법과 실제 개발 과정에서 겪은 문제 해결 과정을 상세히 다룹니다. 
...
![인스타그램 인앱브라우저에서 카카오 로그인 화면](../../contentsImgs/kakao1.png)
...
![카카오 데브톡에서 발견한 인앱브라우저 이슈 관련 게시글](../../contentsImgs/kakao2.png)
...
카카오 로그인에는 **SDK 방식**과 **REST API 방식**이 존재합니다. 그 중에서도 저희는 **카카오 SDK 방식**으로 진행하기로 하였습니다. 

이 두가지 방식의 큰 차이점은 **카카오 SDK 방식**이 카카오 앱이 바로 실행된다는 점에서 차이가 있습니다. REST API를 사용할 경우 서버에서 추가 작업이 소요될 수 있다는 이슈가 있어 **카카오 SDK 방식**을 선택하게 되었습니다. 

# 적용하기

## 스크립트 설정

카카오 개발자 문서에서 제공하는 대로 **카카오 SDK v2**에 맞는 스크립트를 적용하였습니다. 
...
`No sync scripts`라는 오류를 만나게 되었고 **Next.js**에서 발생하는 스크립트 이슈임을 알게되었습니다. **Next.js**에서 권장하는 대로 `Next/Script` 컴포넌트로 변경 후 `beforeInteractive` 옵션을 추가해 줌으로써 **카카오 SDK** 로딩 이슈를 해결하였습니다. 
...
![카카오 로그인 SDK v2 플로우 다이어그램](../../contentsImgs/kakao3.png)
