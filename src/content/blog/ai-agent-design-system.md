...
요청 처리에 필요한 작업 종류가 섞여서, 단계가 케이스마다 바뀌는 상황이면 5번이 더 잘 맞습니다.
- 예: 리서치 → 코드 생성 → 문서 요약 → 정책 체크처럼 task type이 섞이는 경우
- 또 도메인별로 강점이 다른 워커(프롬프트/모델)를 만들어두고 조합하고 싶을 때도 5번 쪽이 자연스럽습니다.

### 언제 6번이 더 적합한가

문제의 핵심이 “어떻게 나눠서 시킬지”보다는, **정확한 외부 사실/상태**를 가져와야 풀리는 경우면 6번이 편합니다.
- 최신 문서 검색, DB 조회, 에러 로그 기반 수정, API 상태 확인 같은 것들이죠.

그리고 툴 결과를 기준으로 추론을 보정해야 할 때(환각 방지/정확도 향상)도 6번이 유리합니다.
